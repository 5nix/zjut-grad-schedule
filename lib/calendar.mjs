...
function dateTimeIso(dateText, timeText) {
  return `${dateText.slice(0, 4)}-${dateText.slice(4, 6)}-${dateText.slice(6, 8)}T${timeText.slice(0, 2)}:${timeText.slice(2, 4)}:00+08:00`;
}
...
export function expandCourses({ studentId, courses, periodsBySemester }) {
  ...
  for (const course of courses) {
      ...
      for (const week of pattern.weeks) {
        ...
        for (const sectionRun of contiguousRuns(pattern.sections)) {
          ...
          courseEvents.push({
            ...
            campus: String(row.XQDM_DISPLAY ?? "").trim(),
    ...
    courseEvents.forEach((event, index) => {
      event.lessonNumber = index + 1;
      event.lessonTotal = lessonTotal;
      event.lessonText = `第${event.lessonNumber}/${lessonTotal}节`;
...
export function toScheduleEvents(events) {
  return events.map((event) => ({
    id: event.uid,
    title: event.title,
    start: dateTimeIso(event.date, event.start),
    end: dateTimeIso(event.date, event.end),
