...
function courseDescription(row, semesterDisplay, schedule) {
  const lines = [];
  const teacher = String(row.RKJS ?? "").trim();
  const campus = String(row.XQDM_DISPLAY ?? "").trim();
  if (teacher) lines.push(`任课教师：${teacher}`);
  if (semesterDisplay) lines.push(`学期：${semesterDisplay}`);
  if (campus) lines.push(`校区：${campus}`);
  if (schedule) lines.push(`周次/节次：${schedule}`);
      ...
      for (const week of pattern.weeks) {
        ...
        for (const sectionRun of contiguousRuns(pattern.sections)) {
          ...
          events.push({
            ...
            description: courseDescription(row, semesterDisplay, row.PKSJ),
          });
        }
      }
    }
